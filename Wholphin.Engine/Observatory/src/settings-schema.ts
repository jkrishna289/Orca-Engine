...
export const GROUPS: Group[] = [
    ...
    fields: [
      {
        ...
        placeholder: 'tmdb,jellyfin,stored,tvdb,search',
        help: 'Comma-separated. tmdb = TMDB videos (language-aware). jellyfin = trailer URLs Jellyfin already found for a library item — free, no key, but library items only. stored = the URL saved on the catalog row. tvdb = TheTVDB trailers, strongest for series. search = search YouTube via yt-dlp and score the results; it sits last so it only runs when everything else came up empty, and it declines rather than guess when nothing scores well. Remove a token to disable that source; unknown tokens are ignored.',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'TrailerSearchCandidates', label: 'Candidates to score', kind: 'int', min: 1, max: 25, help: 'How many search results to fetch before ranking. One yt-dlp call regardless.' },
      { key: 'TrailerScoreTitleMatch', label: '+ Title match', kind: 'int', min: 0, max: 200 },
      { key: 'TrailerScoreYearMatch', label: '+ Year match', kind: 'int', min: 0, max: 200 },
      { key: 'TrailerScoreOfficialChannel', label: '+ Official channel', kind: 'int', min: 0, max: 200, help: 'The channel looks like a studio or distributor, or is named after the title.' },
      { key: 'TrailerScoreOfficialTrailer', label: '+ "Official trailer"', kind: 'int', min: 0, max: 200 },
      { key: 'TrailerScoreTrailer', label: '+ "Trailer"', kind: 'int', min: 0, max: 200 },
      { key: 'TrailerScoreUnrelated', label: '− Title missing', kind: 'int', min: 0, max: 200, help: 'Penalty when the video title does not contain the media title at all.' },
      { key: 'TrailerScoreFanMade', label: '− Fan made', kind: 'int', min: 0, max: 200, help: 'Fan trailers, concept edits, parodies.' },
      { key: 'TrailerScoreClip', label: '− Clip or reaction', kind: 'int', min: 0, max: 200, help: 'Reactions, reviews, breakdowns, featurettes, single scenes.' },
      { key: 'TrailerScoreWrongYear', label: '− Wrong year', kind: 'int', min: 0, max: 200, help: 'The video title carries a year two or more off — usually a different film entirely.' },
    ],
  },
  {
    ...
    fields: [
      ...
      { key: 'OmdbApiKey', label: 'OMDb API key', kind: 'secret', help: 'Free tier, 1000 requests/day. The only source of IMDb / Rotten Tomatoes / Metacritic scores; fills the critic rating nothing has ever populated. Needs an IMDb id, which is resolved from TMDB automatically.' },
      { key: 'FanartApiKey', label: 'Fanart.tv API key', kind: 'secret', help: 'Free personal key. Clear logos and high-resolution artwork. Series need a TVDB id, which is resolved from TMDB automatically.' },
      { key: 'TvdbApiKey', label: 'TheTVDB v4 API key', kind: 'secret', help: 'Series metadata, artwork and trailers. Legacy v1-v3 keys do not work against v4.' },
      { key: 'TvdbPin', label: 'TheTVDB subscriber PIN', kind: 'secret', help: 'Only needed for a user-supported key. Leave empty for a company key — an empty PIN is omitted rather than sent.' },
      { key: 'MetadataLanguage', label: 'Artwork language', kind: 'text', placeholder: 'en', help: 'ISO 639-1. Artwork with text in this language is preferred; textless art ranks next.' },
      { key: 'MetadataPriorityCore', label: 'Priority · overview and genres', kind: 'text', placeholder: 'tmdb,tvdb', help: 'Comma-separated, best first. Removing a token disables that provider for this field.' },
      { key: 'MetadataPriorityArtwork', label: 'Priority · posters and backdrops', kind: 'text', placeholder: 'fanart,tmdb,tvdb', help: 'Artwork is also scored on language and resolution, so priority breaks ties rather than overriding a clearly better image.' },
      { key: 'MetadataPriorityLogo', label: 'Priority · clear logos', kind: 'text', placeholder: 'fanart,tvdb' },
      { key: 'MetadataPriorityRatings', label: 'Priority · ratings', kind: 'text', placeholder: 'omdb,tmdb' },
      { key: 'MetadataPriorityEpisodes', label: 'Priority · episodes', kind: 'text', placeholder: 'tvdb,tmdb' },
      { key: 'MetadataItemsPerPass', label: 'Titles per pass', kind: 'int', min: 1, max: 500, help: 'Maintenance runs every 15 minutes, so 10 costs at most ~960 provider calls a day — just inside OMDb\'s free 1000/day. Raise it only with a paid key.' },
      { key: 'MetadataRefreshDays', label: 'Re-check after (days)', kind: 'int', min: 1, max: 3650, help: 'How long a title is left alone after a metadata pass, successful or not. A title skipped because a provider was rate-limited is not stamped, so it is retried next pass.' },
      { key: 'MetadataCacheDays', label: 'Provider cache (days)', kind: 'int', min: 1, max: 365, help: 'How long one provider\'s answer is reused. A provider that had nothing is remembered for a day.' },
    ],
  },
  {
    ...
    fields: [
      ...
      { key: 'MetadataProviderMaxConcurrency', label: 'Max concurrent calls', kind: 'int', min: 1, max: 16, help: 'Per provider. Applies from the next restart.' },
      { key: 'MetadataProviderMinIntervalMs', label: 'Minimum gap (ms)', kind: 'int', min: 0, max: 60000, help: 'Throttle between two calls to the same provider. 0 disables it.' },
      { key: 'MetadataProviderBreakerThreshold', label: 'Failures before cutout', kind: 'int', min: 1, max: 100, help: 'Consecutive failures that open a provider\'s circuit. A provider with nothing for a title is not a failure.' },
      { key: 'MetadataProviderBreakerCooldownSeconds', label: 'Cutout cooldown (seconds)', kind: 'int', min: 1, max: 3600, help: 'Doubles on each repeat trip, capped at 15 minutes. One probe is admitted when it elapses.' },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        options: [
          { value: 'ollama', label: 'Ollama (local server, no key)' },
      ...
      { key: 'OllamaBaseUrl', label: 'Ollama URL', kind: 'text', placeholder: 'http://localhost:11434' },
      { key: 'OllamaEmbeddingModel', label: 'Ollama model', kind: 'text', placeholder: 'nomic-embed-text' },
      ...
      { key: 'EmbeddingBatchSize', label: 'Batch size', kind: 'int', min: 8, max: 512, help: 'Documents per embedding call when rebuilding the index. Lower it if the provider rejects requests on size or times out. Ignored by TF-IDF, which must see the whole catalog at once.' },
    ],
  },
];
